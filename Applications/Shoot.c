...
void Twist(float Vx, float Vy, float Wz, float pitch_angle, uint16_t shoot_speed)
{
	...
	Gimbal_Ctrl(pitch_angle,Wz,0,1);//云台控制接口
//	Chassis_Task();
}
...
void Chassis_AutoFollow(float Vx, float Vy, float Wz, float pitch_angle, uint16_t shoot_speed)
{
	...
	//相对角度在中间范围内，底盘不动
	if((Yaw_EncodeAngle-90 < GIM_FOLLOW_ANGLE+section_float)		//右半区间，云台右移时区间减小
		&&(Yaw_EncodeAngle-90 > -GIM_FOLLOW_ANGLE+section_float))//左半区间，云台左移时区间减小
	{
		Astrict_Acc(Vx,Vy,0);//加速度限制	,底盘不旋转
		Gimbal_Wz = Wz*FLLOW_RATE;//允许范围内，云台转速*FLLOW_RATE倍
	}
	else if((Yaw_EncodeAngle-90>GIM_FOLLOW_ANGLE+section_float) && (Yaw_EncodeAngle-90<LIM_ANGLE))	//云台在右缓冲区
	{
		if(Wz>0)//云台往外运动
		{
	...
	else if((Yaw_EncodeAngle-90<-GIM_FOLLOW_ANGLE+section_float)&&(Yaw_EncodeAngle-90>(-LIM_ANGLE)))	//云台在左缓冲区
	{
		if(Wz<0)//云台往外运动
		{
			Astrict_Acc(Vx,Vy,Wz*FLLOW_RATE);//加速度限制,底盘以FLLOW_RATE倍速追逐云台
	...
	else if(Yaw_EncodeAngle-90>LIM_ANGLE)	//缓冲范围外右边
	{
		if(Wz>0)//云台往外运动
		{
			Astrict_Acc(Vx,Vy,Wz*FLLOW_RATE);//加速度限制,底盘以FLLOW_RATE倍速追逐云台
		...
		{
			Astrict_Acc(Vx,Vy,0);//加速度限制,底盘等待
			Gimbal_Wz = Wz*FLLOW_RATE;//FLLOW_RATE倍速向范围内运动				// the number 90 in all condition detect are the offset to eliminate the 
		}																																///  error made by previous functions, i cant rebuild the whole part
	}																																	// but with the offset it works well
	else if(Yaw_EncodeAngle-90<-LIM_ANGLE)	//缓冲范围外左边
	{
		if(Wz<0)//云台往外运动
		{
			Astrict_Acc(Vx,Vy,Wz*FLLOW_RATE);//加速度限制,底盘以FLLOW_RATE倍速追逐云台
	...
	Gimbal_Ctrl(pitch_angle,Gimbal_Wz,shoot_speed,0);//云台控制接口
}
