...
void Get_Ctrl_Data(void)
{
	...
	{
		Control_data.Vx = -(float)(RC_Ctl.rc.ch1 - 1024 + RC_ch1_Offset)/660*REMOTE_Vx_MAX;
		Control_data.Vy = (float)(RC_Ctl.rc.ch0 - 1024 + RC_ch0_Offset)/660*REMOTE_Vy_MAX;
	...
	else if(RC_Ctl.rc.s2 == RC_SW_DOWN && RC_Ctl.rc.s1 == RC_SW_MID)//遥控器 左中 右中 云台+底盘平移
	{
		...
		Control_data.Pitch_angle = (float)(RC_Ctl.rc.ch3 - 1024 + RC_ch3_Offset)/1660*REMOTE_Pitch_Angle;/// para modified for test, wait for changing back
		//if (Control_data.Pitch_angle > 25)	Control_data.Pitch_angle = 25;
		//else if (Control_data.Pitch_angle < -25)	Control_data.Pitch_angle = -25;
...
//		else if (Control_data.Yaw_angle < -30)	Control_data.Yaw_angle = -30;
	...
	else if(RC_Ctl.rc.s2 == RC_SW_UP)//电脑
	{
		...
		else//普通
		{
			...
			if (Control_data.Pitch_angle > 20)	Control_data.Pitch_angle = 20;
			else if (Control_data.Pitch_angle < -20)	Control_data.Pitch_angle = -20;
		}
	}
