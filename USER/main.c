...
//	power_ctrl_on(2);//输出3号24V可控电源
	...
	while(1)
	{
		...
		if(Flag500Hz)
		{
			...
			else if(RC_Ctl.rc.s1 == RC_SW_UP && RC_Ctl.rc.s2 == RC_SW_DOWN)//遥控器 左上 右中 底盘控制 底盘跟随云台
			{
				laser_off();
			  //Chassis_Task();
			...
			else if(RC_Ctl.rc.s1 == RC_SW_MID && RC_Ctl.rc.s2 == RC_SW_DOWN){//遥控器 左中 右中 摩擦轮开启 拨弹轮开启 
//				Chassis_Task();
//				shoot_control_loop(Fric_DOWN);
				Chassis_AutoFollow(Control_data.Vx,Control_data.Vy,Control_data.Wz,Control_data.Pitch_angle,20);
//				Twist(float Vx, float Vy, float Wz, float pitch_angle, 50);
//				Gimbal_Ctrl(Control_data.Pitch_angle, Control_data.Yaw_angle, 0);
			}
			else if(RC_Ctl.rc.s1 == RC_SW_DOWN && RC_Ctl.rc.s2 == RC_SW_DOWN){//遥控器 左下 右中 
